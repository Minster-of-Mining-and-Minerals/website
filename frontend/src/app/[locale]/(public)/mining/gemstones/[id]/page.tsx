...
function ExpandableDescription({
    html,
    collapsedMaxHeight = DESCRIPTION_COLLAPSED_HEIGHT,
    proseClassName = "rich-text-content text-gray-600 text-sm leading-relaxed break-words",
}: {
    html: string;
    ...
    useEffect(() => {
        ...
        const checkOverflow = () => {
            if (expanded) return;
            setIsTruncatable(el.scrollHeight > collapsedMaxHeight + 1);
        };
    ...
    }, [html, collapsedMaxHeight, expanded]);

    return (
        <div className="min-w-0">
            <div
                ref={descriptionRef}
                className={`${proseClassName} overflow-hidden transition-[max-height] duration-300 ease-in-out ${
                    expanded ? "max-h-[5000px]" : ""
                }`}
...
function SubClassCard({ item }: { item: Gamestone }) {
    ...
    return (
        <div className="group relative bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col h-full min-w-0">
            {/* Image Container */}
            <div className="relative h-44 sm:h-52 bg-gradient-to-br from-amber-50 to-yellow-100 overflow-hidden">
                {imageUrl ? (
                    <>
                        <img
                            src={imageUrl}
            ...
            {/* Content */}
            <div className="p-4 sm:p-5 flex-grow min-w-0">
                <h4 className="font-bold text-gray-900 text-base sm:text-lg mb-2 line-clamp-2 group-hover:text-amber-700 transition-colors">
                    {item.title}
                </h4>

                {item.location && (
                    <div className="flex items-center gap-1.5 text-gray-500 text-xs mb-2 min-w-0">
...
function DocumentsSection({ attachments }: { attachments: { attachment: any }[] }) {
    ...
    return (
        <div className="bg-gradient-to-br from-white to-amber-50/30 rounded-2xl border border-amber-200/50 shadow-lg p-4 sm:p-6 md:p-8">
            <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-6">
                <div className="p-2 bg-amber-100 rounded-xl shrink-0 w-fit">
                ...
                {attachments.map((a, i) => {
                    ...
                    return (
                        <a
                            ...
                            download={!isPdf}
                            className="group relative flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-white border border-gray-200 rounded-xl px-4 sm:px-5 py-4 hover:shadow-md hover:border-amber-300 transition-all duration-300 overflow-hidden min-w-0"
                        >
                            <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                                <div className="w-11 h-11 sm:w-12 sm:h-12 bg-amber-100 rounded-xl flex items-center justify-center group-hover:bg-amber-200 transition-colors shrink-0">
                                    <FileIcon className="w-5 h-5 text-amber-700" />
                                </div>
                                <div className="min-w-0">
                                    <p className="text-sm font-semibold text-gray-800 group-hover:text-amber-700 transition-colors break-words">
                                        {att.file_name}
                                    </p>
...
function StatsCard({ gemstone }: { gemstone: Gamestone }) {
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            {stats.map((stat, idx) => (
                <div key={idx} className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm hover:shadow-md transition-shadow min-w-0">
    ...
    return (
        <div className="not-prose min-h-screen bg-gradient-to-br from-gray-50 via-white to-amber-50/20">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 relative">
                {/* Breadcrumb */}
                <nav className="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm mb-6 sm:mb-8">
                ...
                <button
                    onClick={() => router.back()}
                    className="group flex items-center gap-2 text-gray-600 hover:text-amber-700 font-medium mb-6 sm:mb-8 transition-colors"
                >
                ...
                <div className="bg-white rounded-2xl border border-gray-200 shadow-xl overflow-hidden mb-8 sm:mb-10">
                    {imageUrl ? (
                        <div className="relative w-full h-48 sm:h-64 md:h-[40vh] lg:h-[45vh] min-h-[12rem] max-h-[32rem]">
                            <img
                                src={imageUrl}
                                alt={gemstone.title}
                            ...
                            <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-6 md:p-8 text-white">
                                <div className="max-w-4xl">
                                    <div className="flex items-center gap-2 mb-2 sm:mb-3">
                                        ...
                                        <span className="text-xs sm:text-sm font-medium text-amber-200">Ethiopian Gemstone</span>
                                    </div>
                        ...
                        <div className="relative w-full h-48 sm:h-64 md:h-[35vh] min-h-[12rem] bg-gradient-to-br from-amber-200 via-amber-100 to-yellow-100 flex items-center justify-center px-4">
                            <Gem className="w-16 h-16 sm:w-24 sm:h-24 text-amber-400/50 absolute" />
                        ...
                        {gemstone.description && (
                            <div className="mb-8 sm:mb-10 min-w-0">
                                <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-4 flex items-center gap-2">
                                    <Eye className="w-5 h-5 sm:w-6 sm:h-6 text-amber-600 shrink-0" />
                                    About this Gemstone
                                </h2>
                                <ExpandableDescription
                                    html={gemstone.description}
                                    collapsedMaxHeight={120}
                                    proseClassName="rich-text-content prose prose-sm sm:prose-base lg:prose-lg max-w-full min-w-0 w-full text-gray-700 leading-relaxed break-words"
                                />
                            </div>
                        )}
                    </div>
